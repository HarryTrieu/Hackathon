...
import { MENTEE_QUESTIONS, SEED_MENTORS, gradeBand, reputationFor } from "./mentors.js";
...
export async function loadListings({ unitCode = null, includePending = false } = {}) {
  ...
  if (db) {
    const [mentorsRes, postsRes, chatsRes, reqRes] = await Promise.all([
      db.from("mentor_profiles").select("*"),
      db.from("posts").select("author_id, helpful_count, status"),
      db.from("mentor_chats").select("mentor_listing"),
      db.from("session_requests").select("mentor_listing"),
    ]);
    ...
    if (!chatsRes.error && chatsRes.data) {
      for (const row of chatsRes.data) {
        if (row.mentor_listing) chatCounts.set(row.mentor_listing, (chatCounts.get(row.mentor_listing) ?? 0) + 1);
      }
    }
    if (!reqRes.error && reqRes.data) {
      for (const row of reqRes.data) {
        if (row.mentor_listing) requestCounts.set(row.mentor_listing, (requestCounts.get(row.mentor_listing) ?? 0) + 1);
      }
    }
  }
  ...
  return [...byId.values()]
    ...
    .map((l) => ({
      ...
      preview_chats: chatCounts.get(l.id) ?? Math.max(3, Math.round(reputationFor(l.profile_id, posts) / 10)),
      contact_requests: requestCounts.get(l.id) ?? Math.max(0, Math.round(reputationFor(l.profile_id, posts) / 40)),
    }))
    .filter((l) => l.profile)
...
export function mockMatch(candidates, messages) {
  ...
  const wantsResults = /show|list|just show|skip|gợi ý|any mentor|just browsing|community only/i.test(last);
  ...
  const scored = candidates.map((l) => {
    ...
    const band = gradeBand(l.grade);
    const reason = hits.length
      ? `${first} matches what you asked for: ${hits.map((h) => h.toLowerCase()).join(", ")}. ${band} in ${l.unit_code}, ${l.reputation} helpful votes in the community${communityOnly ? ", so they already stand out in the free feed" : ""}.`
      : `${first} teaches ${l.style.teaching.toLowerCase()} with a ${l.style.tone.toLowerCase()} tone. ${band} in ${l.unit_code}, ${l.reputation} helpful votes in the community.`;
...
- Never state a mentor's exact grade (HD, D, a mark or a percentage). Say "Distinction or above" if the grade matters.
...
- Grade in the unit: Distinction or above
...
- Visa, enrolment or migration questions: do not advise. Point to Deakin International Student Support and official Home Affairs pages.
- Mental health, self-harm or crisis: do not counsel. Point to Deakin Counselling and Safer Community, and emergency services if someone is in danger.
- Never share a phone number, address, personal socials or invent credentials or work history.
...
export function mockMentorReply(listing, text) {
  ...
  const first = listing.profile.name.split(" ")[0];
  if (/assignment|write my|do my (assignment|essay|report)|give me the answers|solution for the assignment/.test(t)) {
    return `I can't write assessment work for you. That's an academic integrity line at Deakin. I can explain the concept and check your thinking. Which part is confusing?`;
  }
  if (/visa|home affairs|work (hours|rights)|immigration|coe\b/.test(t)) {
    return `I can't advise on visas or work rights. That's official only. Start with Deakin International Student Support and the Department of Home Affairs website.`;
  }
  if (/suicid|kill myself|self[- ]harm|depress|anxious|mental health|want to die/.test(t)) {
    return `I'm not the right support for that. Please contact Deakin Counselling or Safer Community, or call 000 if you are in immediate danger. You matter, and those services are built for this.`;
  }
  if (/phone number|whatsapp|where do you live|address|instagram|facebook/.test(t)) {
    return `I don't share personal contact details. I'm an AI preview of ${first}. If you want to meet, use Request a session on this page.`;
  }
  if (/stupid|idiot|shut up|useless|fuck you/.test(t)) {
    return `I'm here to help with ${listing.unit_code}. If this isn't a fit, try another mentor. What part of the unit is stuck?`;
  }
  const otherUnit = t.match(/\b[a-z]{3}\d{3}\b/);
  if (otherUnit && otherUnit[0].toUpperCase() !== listing.unit_code) {
    return `I only mentor ${listing.unit_code}. For ${otherUnit[0].toUpperCase()}, go back to Find a mentor and pick that unit. I can still help if this question is actually about ${listing.unit_code}.`;
  }
  if (/lost|exam|stress|panic|behind/.test(t)) return listing.voice.lost;
