...
export function rankForYou(posts, persona) {
...
// Popular tab: helpful count, no personalisation, no reason lines.
export function rankPopular(posts) {
  return posts
    .map((post) => ({ post, author: getProfile(post.author_id), reason: null }))
    .sort((a, b) => b.post.helpful_count - a.post.helpful_count);
}
